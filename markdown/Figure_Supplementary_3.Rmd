...
palette_VAF <- c("VAF consistent" = "gray",
                 ...
                 "Allele swapping" = "#86bfd5",
                  "VAF het for hg38 only" = "#C24039",
                 "VAF het for T2T-CHM13 only" = "#ED7D31"
                 )
cbPalette <- c("#D55E00", "#E69F00", "#009E73", "#56B4E9", "#CC79A7")

source("../utilities/theme_ggplot.R")

chr_hg38 <- fread(paste0(utilities_path, "chr_band_hg38.txt"), col.names = c("chrom","chromStart","chromEnd","name","gieStain"))

chr_hg38 <- chr_hg38 %>% mutate(arm=substr(name,1,1)) %>% 
  group_by(chrom,arm) %>% summarize(start=min(chromStart),end=max(chromEnd)) %>%  
  dplyr::rename(chr=chrom) %>% mutate(version=rep("hg38",n())) %>% 
  filter(chr %in% paste0("chr",c(1:22,"X","Y")))

exome_bed_hg38 <- fread(paste0(utilities_path,"sorted_SeqCap_EZ_Exome_v3_hg38_capture_targets.bed"),
                  col.names = c("chr", "start", "end", "name"))

exome_bed_T2T <- fread(paste0(utilities_path, "sorted_SeqCap_EZ_Exome_v3_T2T_capture_targets.bed"),
                  col.names = c("chr", "start", "end", "name"))
...
```{r annotation, fig.width = 7.5, fig.heigth = 2}
...
```{r annotation2, fig.width = 18, fig.heigth = 10}
...
```{r figureS3A, fig.width=9.7, fig.height=9.3}
...
```{r figureS3B, fig.width = 8, fig.height = 8.2}
...
```{r figureS3C, fig.width = 8, fig.height = 5.1}
...
```{r figureS3D, fig.width=10.5, fig.height=3}
...
Plot:
```{r figureS3E, fig.width=9.9, fig.height=5.9}
...
```{r figureS3F, fig.width=9.9, fig.height=7.2}
...
```{r figureS4A, fig.width=8.2, fig.height=7}
(ggplot(all_tracks %>%
...
```{r figureS4B, fig.width=4, fig.height=7}
tp <- cpg_comp %>% 
...
```{r figureS4C, fig.width = 8, fig.height = 8}
...
```{r figureS4D, fig.width= 18.2, fig.height = 8}
...
```{r figureS4E, fig.width=13, fig.height=6}
