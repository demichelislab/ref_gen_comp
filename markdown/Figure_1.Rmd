...
compute_cov_long <- function(comp){
  comp %>% pivot_longer(c(cov.hg38,cov.T2T), names_to = "assembly", values_to = "cov") %>% 
    mutate(assembly = gsub("cov\\.","", assembly)) %>% 
    filter(cov > 60)
} 
...
assign_annotation <- function(df, colname, anno_df) {
 query_gr <- GRanges(
    seqnames = df$chr,          
    ranges   = IRanges(start = df$pos, end = df$pos)
  )
  
 subject_gr <- GRanges(anno_df)
 
 hits <- findOverlaps(query_gr, subject_gr)
 
 hit_df <- data.frame(
    query_idx   = queryHits(hits),
    name        = subject_gr$name[subjectHits(hits)]
  )
 annotations <- hit_df |>
    group_by(query_idx) |>
    summarise(annotation = paste0(name, collapse = "|"), .groups = "drop")
  
  df[[colname]] <- NA_character_
  df[[colname]][annotations$query_idx] <- annotations$annotation
...
```{r figure1B_rasterized, fig.width = 11, fig.height = 5, echo = FALSE}
(ggplot(combined %>% filter(!is.na(value)) %>% group_by(sample, metric) %>% filter(n() == 2) %>% ungroup(), 
        aes(x = assembly, y = value))
  + geom_boxplot(aes(fill = assembly, group = assembly), 
                 position = "dodge", outlier.shape = NA)
  ...
  + geom_line(data = . %>% filter(!grepl("1kGP WGS\nlong-read", design)), 
              aes(x = as.factor(assembly), y = value, group = sample),
               colour = "gray50", alpha = 0.4, position = position_dodge(0.2))
 + ggrastr::rasterise(geom_point(data=. %>% filter(grepl("1kGP WGS\nlong-read", design)),
               aes(x = as.factor(assembly), y = value, fill = assembly, group = sample),
                alpha = 0.3, position = position_dodge(0.2)),  dpi=300)
  + ggrastr::rasterise(geom_line(data = . %>% filter(grepl("1kGP WGS\nlong-read", design)), 
              aes(x = as.factor(assembly), y = value, group = sample),
               colour = "gray50", alpha = 0.2, position = position_dodge(0.2)),  dpi=300)
...
```{r sd_overlap, fig.width = 6, fig.height = 7 }
(ggplot(snps, 
...
```{r Review_2A, fig.width = 8, fig.height = 9 }
...
```{r Review_Fig2, fig.width = 18, fig.height = 8}
...
## Figure S2D
```{r figureS2D, fig.width = 17, fig.height = 4}
...
```{r figureS2E,  fig.width = 18, fig.height = 5.5}
...
```{r figureS2F, fig.width = 18, fig.height = 6}
